...
export function useSimulator() {
  ...
  const additionalCoveragesSum = useMemo(() => {
    return ADDITIONAL_COVERAGES.filter((c) =>
      selectedAdditionalCoverages.includes(c.id)
  ...
  const calculatedPlans = useMemo(() => {
    ...
    return data.plansIndicators.map((plan: PlanIndicator) => {
      ...
      const finalValue =
        plan.value * vehicleMultiplier * ageFactor + additionalCoveragesSum;
  ...
  const recommendedPlanName = useMemo(() => {
    ...
    const profileScore =
      ageRiskScore * 0.35 +
      vehicleRiskScore * 0.4 +
      protectionInterestScore * 0.25;

    const sortedByValue = [...calculatedPlans].sort(
    ...
    if (profileScore < 0.6) return standardPlan.name;
    return premiumPlan.name;
  }, [calculatedPlans, vehicleValue, clientAge, selectedAdditionalCoverages]);

  const selectedPlan = useMemo(() => {
    return (
      calculatedPlans.find(
        (p: PlanIndicator & { finalValue: number }) =>
