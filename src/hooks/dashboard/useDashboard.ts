...
export function useDashboard() {
    ...
    const filteredClientsForMap = useMemo(() => {
        ...
        return data.activeClients.data.filter((client: ActiveClient) => {
            const secureTypeMatch = mapFilters.secureType === 'Todos' || client.secureType === mapFilters.secureType
            const locationMatch = mapFilters.location === 'Todos' || client.location === mapFilters.location
    ...
    const filteredClientsForTable = useMemo(() => {
        ...
        return data.activeClients.data.filter((client: ActiveClient) => {
            const statusMatch = tableFilters.status === 'Todos' || client.status === tableFilters.status
            const secureTypeMatch = tableFilters.secureType === 'Todos' || client.secureType === tableFilters.secureType
            const locationMatch = tableFilters.location === 'Todos' || client.location === tableFilters.location
