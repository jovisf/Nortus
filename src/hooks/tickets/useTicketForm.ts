...
export function useTicketForm({ ticketToEdit, isOpen, onClose }: UseTicketFormProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const submitData = {
            ...formData,
            status,
            ticketId: ticketToEdit?.ticketId || `TK${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
        };

        if (ticketToEdit) {
            updateTicket({ id: ticketToEdit.id, data: submitData as any }, {
                onSuccess: () => onClose()
            });
        } else {
