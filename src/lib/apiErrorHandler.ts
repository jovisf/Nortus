...
import type { ApiError, ErrorHandlerOptions } from '@/types/api';
...
function logError(error: ApiError, context?: Record<string, unknown>): void {
  const errorInfo = {
...
export function handleApiError(
  ...
  switch (statusCode) {
    ...
    case HttpStatus.FORBIDDEN:
      if (showToast) {
        showErrorNotification(customMessage || t('forbidden'));
      }
      break;
    ...
    case HttpStatus.TOO_MANY_REQUESTS:
      if (showToast) {
        showErrorNotification(customMessage || t('tooManyRequests'));
      }
      break;
