...
export const getPriorityStyles = (priority: string) => {
    switch (priority) {
        ...
        case 'Média': return 'bg-priority-medium text-black';
        case 'Baixa': return 'bg-priority-low text-black';
...
export const getStatusStyles = (status: string) => {
    switch (status) {
        case 'Em andamento': return 'bg-status-inprogress text-black';
        case 'Aberto': return 'bg-status-open text-black';
        case 'Fechado': return 'bg-success text-black';
        default: return 'bg-white/10 text-white';
    }
};
